...
export default {
    ...
    initEditor() {
      return {
        ...
        images_upload_handler: (blobInfo, success, error) => {
          ...
          this.$http.post('/oss/upload', formData).then(res => {
  ...
  methods: {
    ...
    getInfo(){
      if (this.data.id > 0) {
        this.$http.get('/cfProtocols/info/' + this.data.id).then(res => {
          if (res.data.code === 200) {
    ...
    save() {
      this.$refs['form'].validate((valid) => {
        if (valid) {
          this.loading = true;
          this.$http.post(this.isUpdate ? '/cfProtocols/edit' : '/cfProtocols/add', this.form).then(res => {
            this.loading = false;
            if (res.data.code === 200) {
