...
export default {
  ...
  methods: {
    ...
    remove(row) {
      ...
      this.$http.delete('/example2/delete', {id: row.id}).then(res => {
        loading.close();
        if (res.data.code === 0) {
    ...
    removeBatch() {
      ...
      this.$confirm('确定要删除选中的演示案例二吗?', '提示', {
        ...
        const loading = this.$loading({lock: true});
        this.$http.delete('/example2/delete', {id: this.selection.map(d => d.id)}).then(res => {
          loading.close();
          if (res.data.code === 0) {
            this.$message.success(res.data.msg);
            this.reload();
    ...
    editStatus(row) {
      ...
      this.$http.put('/example2/setStatus', params).then(res => {
        loading.close();
        if (res.data.code === 0) {
          this.$message.success(res.data.msg);
        } else {
