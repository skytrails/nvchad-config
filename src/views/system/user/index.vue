        ...
        <template slot="action" slot-scope="{row}">
          ...
          <el-popconfirm
            ...
            <el-link
              ...
              slot="reference"
              :underline="false"
              icon="el-icon-delete" v-if="row.id !== 1">删除
            </el-link>
          </el-popconfirm>
          <el-popconfirm
            class="ele-action"
            ...
            <el-link
              ...
              slot="reference"
              :underline="false"
              icon="el-icon-copy-document" v-if="row.id !== 1">重置密码
            </el-link>
          </el-popconfirm>
        </template>
      </ele-pro-table>
...
export default {
  ...
  methods: {
    ...
    remove(row) {
      ...
      this.$http.post('/sysUser/delete', [row.id]).then(res => {
        loading.close();
    ...
    removeBatch() {
      ...
      this.$confirm('确定要删除选中的用户吗?', '提示', {
        ...
        const loading = this.$loading({lock: true});
        this.$http.post('/sysUser/delete', this.selection.map(d => d.id)).then(res => {
          loading.close();
          if (res.data.code === 200) {
    ...
    editStatus(row) {
      ...
      this.$http.post('/sysUser/status', params).then(res => {
        loading.close();
        if (res.data.code === 200) {
          this.$message({type: 'success', message: res.data.message});
        } else {
    ...
    resetPwd(row){
      const loading = this.$loading({lock: true});
        this.$http.post('/sysUser/resetPwd', {id: row.id}).then(res => {
          loading.close();
          if (res.data.code === 200) {
            this.$message({type: 'success', message: res.data.message});
          } else {
